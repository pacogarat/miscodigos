angular.module('xenon.directives', []).
        directive('select2', function($log)   {
            return {
                restrict: 'AEC',
                transclude: true,
                scope: {
                ...
                link: function(scope, element, attrs) {
                    ...
                    scope.$watch('jsonInit', function(){
                        scope.update();
                    });
                    scope.update = function() {
                        $(element).children('input').select2({
                            ...
                            initSelection: function (element, callback) {
                                if (scope.jsonInit!=undefined) callback(scope.jsonInit);
                            },
                            ...
                            formatSelection: function(object, container) {
                                if (scope.model==undefined) scope.model=[];
                                var esta = $.inArray(object, scope.model);
                                if(esta == -1){
                            ...
                            var index = scope.model.indexOf(e.choice);
                            if (index > -1) {
                                scope.model.splice(index, 1);
                            }
                        }).on("select2-added", function(e) {
                            scope.model.push(e);
                        }).on("select2-selecting", function(e) {
                            var a = scope.onselect();
                            a(e.object);
                        }).on("select2-removed", function(e) {
                            var a = scope.onremove();
                            a(e.object);
                        }).select2('val', scope.jsonInit);
                    };
                    scope.update();
                }
            };
	...
	directive('wysihtml5', function($ocLazyLoad, ASSETS){
		return {
			restrict: 'AC',
			link: function(scope, el, attr)
			{
